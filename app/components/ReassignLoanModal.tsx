...
export default function ReassignLoanModal({
  isOpen,
  onClose,
  onSuccess,
  loan,
}: ReassignLoanModalProps) {
  ...
  const handleReassign = async () => {
    ...
    const result = await reassignLoanUseCase.execute(
      String(loan.id),
      selectedCollectorId,
    );
      ...
      >
        ...
        >
          ...
          <div style={{ color: "var(--text-secondary)", fontSize: "0.8rem" }}>
            ID: #{loan.id} | Cobrador actual: {loan.collectorName}
          </div>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          ...
          <div>
            ...
            >
              <option value="">
                {loading ? "Cargando..." : "Seleccionar cobrador..."}
              </option>
              {collectors.map((c) => (
                <option key={c.id} value={c.id}>
