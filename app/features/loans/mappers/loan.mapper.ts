...
export class LoanMapper {
  ...
  static toDashboardDataDomain(dto: DashboardDataDto): DashboardData {
    return {
      ...
      pendingLoans: dto.pendingLoans
        ? dto.pendingLoans.map((loan) => this.toDashboardDomain(loan))
        : [],
  ...
  static toInstallmentDetailDomain(
    dto: InstallmentDetailDto,
  ): InstallmentDetail {
    return {
      id: dto.id,
      date: dto.date,
  ...
  static toLoanDetailsDomain(dto: LoanDetailsDto): LoanDetails {
    return {
      ...
      installments: dto.installments
        ? dto.installments.map((i) => this.toInstallmentDetailDomain(i))
        : [],
    };
  }
  ...
  static toReportDataDomain(dto: ReportDataDto): ReportData {
    return {
      ...
      pagosPorDia: dto.pagosPorDia
        ? dto.pagosPorDia.map((day) => ({
            fecha: day.fecha,
            gastos: day.gastos || [],
            pagos: day.pagos
              ? day.pagos.map((payment) => ({
